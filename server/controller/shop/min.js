    ...
    if(cate === undefined)
    {
        ...
        else if(price==="10")
        {
            await db.sequelize.query(
                ...
                'WHERE b.PRODUCT_KEY = a.PRODUCT_KEY \n' +
                'AND b.SELL_PRICE = (SELECT MIN(SELL_PRICE) FROM Sell WHERE PRODUCT_KEY = a.PRODUCT_KEY)\n'+
                'AND b.SELL_PRICE < 100000 ;', { type: sequelize.QueryTypes.SELECT })
                .then(result => {
                    console.log(result);
                    res.json(result);
                })
        ...
        else if(price==="30")
        {
            await db.sequelize.query(
                ...
                'WHERE b.PRODUCT_KEY = a.PRODUCT_KEY \n' +
                'AND b.SELL_PRICE = (SELECT MIN(SELL_PRICE) FROM Sell WHERE PRODUCT_KEY = a.PRODUCT_KEY)\n'+
                'AND b.SELL_PRICE BETWEEN 100000 AND 300000;', { type: sequelize.QueryTypes.SELECT })
                .then(result => {
                    console.log(result);
                    res.json(result);
                })
        ...
        else if(price==="50")
        {
            await db.sequelize.query(
                ...
                'WHERE b.PRODUCT_KEY = a.PRODUCT_KEY \n' +
                'AND b.SELL_PRICE = (SELECT MIN(SELL_PRICE) FROM Sell WHERE PRODUCT_KEY = a.PRODUCT_KEY)\n'+
                'AND b.SELL_PRICE BETWEEN 300001 AND 500000;', { type: sequelize.QueryTypes.SELECT })
                .then(result => {
                    console.log(result);
                    res.json(result);
                })
            ...
            'WHERE b.PRODUCT_KEY = a.PRODUCT_KEY \n' +
            'AND b.SELL_PRICE = (SELECT MIN(SELL_PRICE) FROM Sell WHERE PRODUCT_KEY = a.PRODUCT_KEY)\n'+
            'AND b.SELL_PRICE > 500000;', { type: sequelize.QueryTypes.SELECT })
            .then(result => {
                console.log(result);
                res.json(result);
            })
