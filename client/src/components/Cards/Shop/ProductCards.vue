    ...
    >
      <div class="flex-auto p-4">
        <div
          ...
          @click="goView(`${PRODUCT_KEY}`)"
          style="cursor: pointer"
        >
          <div class="relative w-full max-w-full flex-grow flex-1">
            <div
              class="w-full mb-5"
              style="overflow: hidden; width: 205px; height: 205px"
            >
              <img
                :src="imageUrl + PRODUCT_PIC"
                alt="아이템 사진"
                style="
                  object-fit: cover;
        ...
        <div v-if="!this.likeStatus">
          <v-icon style="font-size: 20px" @click="goLike()">
            mdi-heart-outline
          </v-icon>
          <span class="text-xs">{{ likeTotal }}</span>
        </div>
        <div v-else>
          <v-icon style="font-size: 20px" @click="goDislike()">
            mdi-heart
          </v-icon>
          <span class="text-xs">{{ likeTotal }}</span>
        </div>
