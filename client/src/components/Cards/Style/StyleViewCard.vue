  ...
  <v-container class="border mb-24">
    <v-row justify="space-around">
      <v-card class="w-full pl-10 pr-10" :elevation="0">
        <img :src="imageUrl + POST_PIC" crossorigin />
        <v-card-actions>
          <v-card-text>
            <div class="mb-10">
              <span
                class="font-semibold text-blueGray-400 ml-1"
                v-for="(ht, idx) in hashtag"
                :key="idx"
              >
                #{{ ht }}
              </span>
            ...
            <div class="mt-8">
              <v-btn icon @click="showComment()">
                <v-icon>{{
                  show ? 'mdi-comment' : 'mdi-comment-outline'
                }}</v-icon>
              ...
              >
                <v-icon>{{
                  show ? 'mdi-delete-empty' : 'mdi-delete-empty-outline'
                }}</v-icon>
                삭제
              </v-btn>
            </div>
        ...
        <v-expand-transition>
          <div v-show="show">
            ...
            <v-row class="mt-5">
              ...
              ></v-textarea>
              <v-icon @click="addComment(`${POST_KEY}`)"
...
export default {
  ...
  methods: {
    deletePost(POST_KEY) {
      ...
      if (
        confirm(
          '정말로 삭제하시겠습니까? 한번 삭제한 게시물은 되돌릴 수 없습니다.'
        )
      ) {
        that.$axios
          .post(`${this.$store.getters.ServerUrl}/style/deletePost`, {
            POST_KEY: POST_KEY
          })
          .then(function () {
            alert('게시글을 삭제했습니다.');
    ...
    addComment(POST_KEY) {
      ...
      that.$axios
        .post(`${this.$store.getters.ServerUrl}/style/addComment`, {
          POST_KEY: POST_KEY,
    ...
    countPost() {
      this.$axios
        .post(`${this.$store.getters.ServerUrl}/style/addComment/count`, {
    ...
    loadComment() {
      ...
      that.$axios
        .post(`${this.$store.getters.ServerUrl}/style/loadComment`, {
          POST_KEY: this.POST_KEY
        })
        .then(function (res) {
