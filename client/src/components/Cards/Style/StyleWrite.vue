  ...
  <v-row justify="center" class="mt-2">
    ...
    >
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="red darken-1" text @click="$emit('sendDialog')">
            취소
          </v-btn>
          <v-btn color="blue darken-1" text @click="saveStyleWrite">
            저장
          </v-btn>
        </v-card-actions>
      </v-card>
...
export default {
  ...
  methods: {
    ...
    saveStyleWrite() {
      let that = this;
      that.$axios
        .post(`${this.$store.getters.ServerUrl}/style/addPost`, {
      ...
      this.$axios
        .post(`${this.$store.getters.ServerUrl}/uploadStyleImage`, formData, {
          headers: {
