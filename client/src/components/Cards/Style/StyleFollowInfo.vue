  ...
  <v-card class="mx-auto w-full" outlined>
    <v-list three-line class="mt-4">
      <v-list-item>
        ...
        <v-list-item-action>
          <v-btn outlined rounded text @click="fDialog()">
            게시글 올리기
          </v-btn>
        </v-list-item-action>
      </v-list-item>

      <small class="block text-center text-blueGray-400"
        >프로필 수정은 마이페이지에서 진행해주세요.</small
      >
    ...
    <v-dialog v-model="dialog" min-width="290" max-width="350">
      <v-card class="p-4">
        <span class="text-md font-semibold" v-if="follower === true">
          FOLLOW
        </span>
        <span class="text-md font-semibold" v-else> FOLLOWING </span>
        <div v-if="follower">
...
export default {
  ...
  methods: {
    ...
    countPost() {
      let that = this;
      that.$axios
        .post(`${this.$store.getters.ServerUrl}/style/loadPost/count`, {
    ...
    follow() {
      let that = this;
      that.$axios
        .post(`${this.$store.getters.ServerUrl}/style/follow/count/following`, {
          user_key: that.user_key
        })
        .then((res) => {
      ...
      this.$axios
        .post(`${this.$store.getters.ServerUrl}/style/follow/count/follower`, {
          user_key: that.user_key
        })
        .then((res) => {
      ...
      this.$axios
        .post(`${this.$store.getters.ServerUrl}/style/follow/loadFollower`, {
          user_key: that.user_key
        })
        .then((res) => {
      ...
      this.$axios
        .post(`${this.$store.getters.ServerUrl}/style/follow/loadFollowing`, {
          user_key: that.user_key
        })
        .then((res) => {
