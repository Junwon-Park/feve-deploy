...
export default {
  ...
  created() {
    let that = this;
    this.$axios
      .post(`${this.$store.getters.ServerUrl}/shop/shoplist/totalCnt`)
      .then(function (res) {
        ...
        console.log(
          '페이지 버튼 개수: ',
          Math.ceil(that.totalListCount / that.itemPerPage)
        );
      })
      .catch(function (err) {
        console.log(err);
  ...
  methods: {
    getProductList: function () {
      let that = this;
      this.$axios
        .post(`${this.$store.getters.ServerUrl}/shop/shoplist/productlist`, {
          limitStart: 0,
          limitEnd: 8
        })
        .then(function (res) {
    ...
    goFilter(cate) {
      ...
      //console.log(cate);
      this.$axios
        .post(`${this.$store.getters.ServerUrl}/shop/filterCate`, {
          cate: cate
        })
        .then((res) => {
