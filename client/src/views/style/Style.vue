    ...
    <v-card :elevation="0">
      ...
      <v-toolbar color="white" flat prominent>
        ...
        <template v-slot:extension>
          <v-tabs v-model="tabs" centered color="black">
            <v-tab v-for="(tabTitle, idx) in tabTitle" :key="idx">
              {{ tabTitle }}
            </v-tab>
          </v-tabs>
        </template>
      ...
      <v-tabs-items v-model="tabs" class="mt-24">
        <v-tab-item>
          <div class="w-full px-4 mb-5 flex flex-wrap">
            <div
              class="w-full lg:w-6/12 xl:w-3/12 px-4 mb-5"
              v-for="(item, i) in newitems"
              :key="i"
            >
              <CardStyle v-bind="item" :items="item" />
            </div>
          </div>
        </v-tab-item>
        ...
        <v-tab-item>
          <div class="w-full px-4 mb-5 flex flex-wrap justify-center">
            <div class="w-full lg:w-4/12 xl:w-4/12 px-4 mb-5">
              <UserInfo :userInfo="userInfo" @sendDialog="cDialog" />
            </div>
          </div>
          <div class="w-full px-4 my-10 flex flex-wrap">
            <div
...
export default {
  ...
  methods: {
    getSimpleUserInfo() {
      this.$axios
        .post(`${this.$store.getters.ServerUrl}/mypage/getSimpleUserInfo`, {
    ...
    loadMyPost() {
      this.$axios
        .post(`${this.$store.getters.ServerUrl}/style/loadPost`, {
          USER_KEY: JSON.parse(localStorage.getItem('userKey'))
        })
        .then((res) => {
    ...
    loadFollowPost() {
      this.$axios
        .post(`${this.$store.getters.ServerUrl}/style/loadPost/follow`, {
          USER_KEY: JSON.parse(localStorage.getItem('userKey'))
        })
        .then((res) => {
          this.followitems = res.data;
    ...
    loadAllPost() {
      this.$axios
        .post(`${this.$store.getters.ServerUrl}/style/loadPost/all`, {
          USER_KEY: JSON.parse(localStorage.getItem('userKey'))
        })
        .then((res) => {
